...
`src/lib/ranking.ts` is pure and unit-tested — keep scoring logic there rather than in route handlers. `for-you` blends learned affinity, selected-topic match, 48-hour half-life recency, engagement, source diversity, and a deterministic exploration hash; `trending` drops undated items; `latest` sorts by date with unknown dates last. `GET /api/feed` then excludes saved and submitted items from `for-you` (they live on the Saved page), dedupes eligible IDs before ranking, and exempts saved items from the recency window in `trending` and `latest`. Responses are `Cache-Control: private, no-store`.
