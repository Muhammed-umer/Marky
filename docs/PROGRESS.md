...
| Community-tier ingestion: Medium tag feeds | Seeded, not yet live. 10 tag feeds as `rss` + `trust_tier='community'`, through the generic adapter and its qualification gate. Resend and Neon deliberately excluded (alias collides with an everyday word) | `supabase/migrations/20260909020000_add_medium_tag_sources.sql` |
| Source trust in ranking | Done. `FeedItem.trust` carried from `sources.trust_tier`; multiplier applied to For You and Trending, `latest` left a pure date sort | `src/lib/ranking.ts`, `src/app/api/feed/route.ts` |
| Source-page enrichment at ingest (image + body from `og:image`/JSON-LD/Readability) | Done, unit-tested decision/merge. Fires before qualification for new entries with no image or a body under 400 words; capped 12 per run | `src/lib/ingestion/enrich.ts`, `src/lib/ingestion/generic-rss.ts` |
| Article body in the reader | Done. Loaded on demand for the open brief via `GET /api/items/[id]`; summary + link only in demo/signed-out | `src/app/api/items/[id]/route.ts`, `src/components/marky-app.tsx` |
| Honest reading time | Done. From `content_items.word_count` (generated column); omitted when no body | `20260909030000_add_content_item_word_count.sql` |
| Canonical URL, SHA-256 hash, global deduplication | Done | `src/lib/url.ts`, ingestion adapters |
| Near-duplicate removal from stored rows | Done, **not yet run against the live project** — dry run by default; merges references onto the survivor before deleting | `src/lib/dedupe.ts`, `src/lib/ingestion/dedupe-store.ts`, `POST /api/cron/dedupe` |
| Deterministic keyword classification (12 direct entity topics) | Implemented and unit-tested | `src/lib/ingestion/classify.ts` |
| Publisher images (any host, og:image + JSON-LD) | Done | `src/lib/ingestion/rss.ts`, `src/lib/ingestion/web-metadata.ts` |
...
| Free discovery adapters (Discovery Map Step 3) | Done in code, **migration not applied** — `hacker_news`, `dev_to`, `stack_exchange`, `github_releases`, `youtube` behind one interface; 28 sources seeded, 0 YouTube (needs real channel ids) | `src/lib/ingestion/discovery.ts` and the per-platform modules |
| Discovery quota ledger | Done in code, **migration not applied** — Stack Exchange 200/day, GitHub 40/hour, row-locked | `supabase/migrations/20260910000000_add_free_discovery_adapters.sql`, `src/lib/ingestion/quota.ts` |
| X / paid source adapters | **Not started** — gap-fill only, behind a flag, per the Discovery Map | none |
| Automated security, RLS, and browser tests | **Not started** | 19 unit-test files (168 tests) only |
