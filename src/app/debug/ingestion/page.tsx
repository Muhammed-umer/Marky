...
import { QUALIFICATION_THRESHOLD } from "@/lib/qualification";
...
export default async function IngestionDebugPage() {
  const { scheduler, auth, recentRuns, topics, rejections } = await getIngestionAuditData();
  ...
  return (
    <div style={{ padding: "40px 24px", maxWidth: "1400px", margin: "0 auto", fontFamily: "system-ui, -apple-system, sans-serif", color: "#242424", background: "#fbf9f5", minHeight: "100vh" }}>
      ...
      <section style={{ padding: "28px", borderRadius: "12px", background: "#ffffff", border: "1px solid #e7e7e4", marginBottom: "36px", boxShadow: "0 2px 8px rgba(0,0,0,0.04)" }}>
        <h2 style={{ margin: "0 0 6px", fontSize: "20px", fontFamily: "Georgia, serif" }}>3b. Qualification Reject Log</h2>
        <p style={{ margin: "0 0 16px", color: "#6b6b67", fontSize: "13px" }}>
          Last 50 candidates scored below {QUALIFICATION_THRESHOLD} or hard-rejected before storage. Read this before tuning a weight — if the rules are failing, the reasons here are the evidence.
        </p>
        {rejections.length === 0 ? (
          <p style={{ margin: 0, color: "#6b6b67", fontSize: "13px" }}>No rejections recorded yet. This stays empty until the qualification migration is applied and ingestion runs.</p>
        ) : (
          <>
            <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", marginBottom: "16px" }}>
              {Object.entries(rejections.reduce<Record<string, number>>((counts, row) => {
                counts[row.reason] = (counts[row.reason] ?? 0) + 1;
                return counts;
              }, {})).sort((a, b) => b[1] - a[1]).map(([reason, count]) => (
                <span key={reason} style={{ padding: "4px 10px", borderRadius: "12px", background: "#f3f4f6", color: "#4b5563", fontSize: "12px", fontWeight: 600 }}>
                  {reason} · {count}
                </span>
              ))}
            </div>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px", textAlign: "left" }}>
                <thead>
                  <tr style={{ borderBottom: "2px solid #e7e7e4", color: "#6b6b67" }}>
                    ...
                    <th style={{ padding: "10px 12px" }}>Logged (UTC)</th>
                  </tr>
                </thead>
                <tbody>
                  {rejections.map((row) => (
                    <tr key={`${row.canonicalUrl}-${row.createdAt}`} style={{ borderBottom: "1px solid #f0f0ee" }}>
                      <td style={{ padding: "10px 12px", maxWidth: "420px" }}>
                        <a href={row.canonicalUrl} target="_blank" rel="noreferrer" style={{ color: "#1e6a85" }}>{row.title}</a>
                      </td>
                      <td style={{ padding: "10px 12px" }}>{row.sourceName}</td>
                      <td style={{ padding: "10px 12px", fontVariantNumeric: "tabular-nums" }}>{row.score === null ? "—" : row.score.toFixed(3)}</td>
                      <td style={{ padding: "10px 12px" }}>{statusBadge(row.reason)}</td>
                      <td style={{ padding: "10px 12px", color: "#6b6b67" }}>{new Date(row.createdAt).toISOString().replace("T", " ").slice(0, 19)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
