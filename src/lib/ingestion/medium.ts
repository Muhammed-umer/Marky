...
export function imageUrlFromHtml(html: string): string | null {
  ...
  for (const tag of metaTags) {
    ...
    const imageUrl = mediumImageUrl(htmlAttribute(tag, "content") ?? "");
  ...
  for (const tag of imageLinks) {
    ...
    const imageUrl = mediumImageUrl(htmlAttribute(tag, "href") ?? "");
    if (imageUrl) return imageUrl;
  }
...
export async function ingestMediumSource(supabase: SupabaseClient, source: MediumSource, interestRows: InterestRow[]): Promise<SourceResult> {
  ...
  try {
    ...
    // The same gate the generic RSS adapter runs. Without it this adapter
    // stored anything recent, near-duplicate or not -- which is why the Medium
    // tag feeds were seeded as source_type 'rss' rather than routed here.
    ...
    // Titles accepted earlier in this run are not stored yet, but one feed can
    // still offer two near-identical items in a single batch.
    ...
    for (const { candidate } of candidates) {
      ...
      const verdict = qualifyCandidate(
        ...
        { aliases: qualificationContext.aliases, trust: qualificationContext.trust, recentTitles: seenTitles },
      );
    ...
    // Written before any insert so a later failure cannot lose the record of
    // what the gate decided.
    await logQualificationBatch(supabase, verdictLog, source.id);

    // Sources run four-wide and each took its title snapshot before any of them
    // inserted anything, so a sibling run can have stored the same story in the
    // meantime -- the window that put one Medium post in the table three times
    // on 2026-09-10, once per tag feed carrying it. The generic RSS and
    // discovery adapters already close it; this one did not.
    if (accepted.size) {
    ...
    for (const { candidate, metadata } of candidates) {
      ...
      if (contentItemId) {
        ...
        // A rejected candidate is never stored, and never linked to a topic.
