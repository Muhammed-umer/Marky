...
async function fetchFeed(source: IngestionSource) {
  ...
  const url = await assertPublicHttpUrl(source.feed_url);
  ...
  const response = await fetch(url, { headers, redirect: "manual", signal: AbortSignal.timeout(FETCH_TIMEOUT_MS), cache: "no-store" });
  if (response.status >= 300 && response.status < 400) throw new Error("FEED_REDIRECT_REJECTED");
...
export async function ingestGenericRssSource(supabase: SupabaseClient, source: IngestionSource, interests: InterestRow[]): Promise<SourceResult> {
  ...
  try {
    ...
    for (const candidate of candidates) {
      ...
      if (contentItemId) duplicates += 1;
      else {
        const { data: created, error } = await supabase.from("content_items").insert({
          primary_source_id: source.id, external_id: candidate.externalId, canonical_url: candidate.canonicalUrl,
          url_hash: hash, title: candidate.title, author: candidate.author, summary: candidate.summary,
          published_at: candidate.publishedAt, publication_time_status: "known",
        }).select("id").single();
        if (error?.code === "23505") {
          ...
          duplicates += 1;
        } else if (error || !created) throw new Error("ITEM_INSERT_FAILED");
        else { contentItemId = created.id as string; inserted += 1; }
      }
      if (!contentItemId) throw new Error("ITEM_ID_MISSING");
      const { error: sourceError } = await supabase.from("content_item_sources").upsert({ content_item_id: contentItemId, source_id: source.id }, { onConflict: "content_item_id,source_id", ignoreDuplicates: true });
      if (sourceError) throw new Error("SOURCE_LINK_FAILED");
      const links = classifyContent(candidate.title, candidate.summary, source.name).flatMap((match) => {
        const interest = interests.find((row) => row.name === match.name);
        return interest ? [{ content_item_id: contentItemId, interest_id: interest.id, confidence: match.confidence }] : [];
      });
      if (links.length) {
        const { error } = await supabase.from("content_item_interests").upsert(links, { onConflict: "content_item_id,interest_id" });
        if (error) throw new Error("INTEREST_LINK_FAILED");
      }
    }
