...
async function persistArticle(client: SupabaseClient, message: z.infer<typeof queueMessageSchema>): Promise<FeedItem> {
  ...
  const result = existing
    ...
    : await client.from("content_items").insert({ ...values, fetched_at: new Date().toISOString() }).select("id").single();
  ...
  return {
    ...
    imageUrl: null,
    engagementCount: 0,
    interests: (matches.length ? matches.map((match) => match.name) : ["Developer Tools"]) as Interest[],
    sourceCount: 1,
    saved: true,
    read: false,
