...
async function performLiveFetchDiagnostic(
  ...
  try {
    ...
    for (const candidate of parsedCandidates.slice(0, 10)) {
      ...
      const classificationMatches = classifyContent(candidate.title, candidate.summary, candidate.bodyText);
...
export async function getIngestionAuditData(): Promise<{
  ...
  if (!admin) {
    return { scheduler, auth, recentRuns: [], topics: [], rejections: [] };
  }
  ...
  for (const topic of topicsList) {
    const matchingSource = sourcesList.find((s) => {
      // Official feeds only. The match is a substring test, so "Medium — OpenAI"
      // also contains "openai" and sorts ahead of "OpenAI News" -- without this
      // the audit would silently report on the community feed instead of the
      // official blog it is meant to check.
      if ((s.trust_tier ?? "official") !== "official") return false;
  ...
  // The reject log is the feedback loop for the qualification rules: read it
  // before changing a weight or reaching for a smarter model.
  const { data: rejectionRows } = await admin
    .from("discovery_candidates")
    .select("title,canonical_url,score,reason,created_at,source:sources(name)")
    .eq("decision", "rejected")
    .order("created_at", { ascending: false })
    .limit(50);

  const rejections: RejectionRecord[] = (rejectionRows ?? []).map((row) => {
    const source = Array.isArray(row.source) ? row.source[0] : row.source;
    return {
      title: row.title as string,
      canonicalUrl: row.canonical_url as string,
      sourceName: (source as { name?: string } | null)?.name ?? "Unknown source",
  ...
  return { scheduler, auth, recentRuns, topics: topicAudits, rejections };
}
